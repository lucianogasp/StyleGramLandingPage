...
//#region node_modules/turbo-stream/dist/turbo-stream.mjs
...
function raceSignal(promise, signal) {
	...
	if (signal.aborted) return Promise.reject(signal.reason || /* @__PURE__ */ new Error("Signal was aborted."));
...
//#endregion
//#region node_modules/react-router/dist/development/chunk-K6CSEXPM.mjs
...
function createRouter(init) {
	...
	async function handleFetcherAction(key, routeId, path, match, requestMatches, scopedContext, isFogOfWar, flushSync, preventScrollReset, submission) {
		...
		let actionResult = (await callDataStrategy("action", fetchRequest, [match], requestMatches, scopedContext, key))[match.route.id];
	...
	async function handleFetcherLoader(key, routeId, path, match, matches, scopedContext, isFogOfWar, flushSync, preventScrollReset, submission) {
		...
		fetchControllers.set(key, abortController);
		let originatingLoadId = incrementingLoadId;
		let result = (await callDataStrategy("loader", fetchRequest, [match], matches, scopedContext, key))[match.route.id];
		if (fetchControllers.get(key) === abortController) fetchControllers.delete(key);
	...
	async function callLoadersAndMaybeResolveData(matches, matchesToLoad, fetchersToLoad, request, scopedContext) {
		...
		let fetcherResultsPromise = Promise.all(fetchersToLoad.map(async (f) => {
			if (f.matches && f.match && f.controller) {
				let result = (await callDataStrategy("loader", createClientSideRequest(init.history, f.path, f.controller.signal), [f.match], f.matches, scopedContext, f.key))[f.match.route.id];
				return { [f.key]: result };
	...
	function patchRoutes(routeId, children) {
		let isNonHMR = inFlightDataRoutes == null;
		patchRoutesImpl(routeId, children, inFlightDataRoutes || dataRoutes, manifest, mapRouteProperties2);
		if (isNonHMR) {
...
function createStaticHandler(routes, opts) {
		...
		if (respond && matches.some((m) => m.route.unstable_middleware)) {
			...
			try {
				...
				let response = await runMiddlewarePipeline({
					...
					if (isResponse(e.error)) return e.error;
					if (renderedStaticContext) {
						if (e.routeId in renderedStaticContext.loaderData) renderedStaticContext.loaderData[e.routeId] = void 0;
						...
						return respond({
							...
							loaderData: {},
							actionData: null,
							errors: { [boundary.route.id]: e.error },
	...
	async function queryRoute(request, { routeId, requestContext, dataStrategy, unstable_respond: respond } = {}) {
		...
		if (respond && matches.some((m) => m.route.unstable_middleware)) {
			...
			return await runMiddlewarePipeline({
				...
				if (isResponse(e.error)) return respond(e.error);
	...
	async function submit(request, matches, actionMatch, requestContext, dataStrategy, skipLoaderErrorBubbling, isRouteRequest, filterMatchesToLoad, skipRevalidation) {
		...
		if (!actionMatch.route.action && !actionMatch.route.lazy) {
			...
			result = (await callDataStrategy("action", request, [actionMatch], matches, isRouteRequest, requestContext, dataStrategy))[actionMatch.route.id];
	...
	async function loadRouteData(request, matches, requestContext, dataStrategy, skipLoaderErrorBubbling, routeMatch, filterMatchesToLoad, pendingActionResult) {
		...
		let matchesToLoad = (routeMatch ? [routeMatch] : pendingActionResult && isErrorResult(pendingActionResult[1]) ? getLoaderMatchesUntilBoundary(matches, pendingActionResult[0]) : matches).filter((m) => (m.route.loader || m.route.lazy) && (!filterMatchesToLoad || filterMatchesToLoad(m)));
...
function patchRoutesImpl(routeId, children, routesToUse, manifest, mapRouteProperties2) {
	...
	let newRoutes = convertRoutesToDataRoutes(children.filter((newRoute) => !childrenToPatch.some((existingRoute) => isSameRoute(newRoute, existingRoute))), mapRouteProperties2, [
		routeId || "_",
		"patch",
...
async function defaultDataStrategyWithMiddleware(args) {
	...
	return runMiddlewarePipeline(args, false, () => defaultDataStrategy(args), (e) => ({ [e.routeId]: {
		type: "error",
		result: e.error
	} }));
}
...
async function callDataStrategyImpl(dataStrategyImpl, type, request, matchesToLoad, matches, fetcherKey, manifest, mapRouteProperties2, scopedContext, enableMiddleware) {
	...
	if (enableMiddleware) await Promise.all(loadRouteDefinitionsPromises);
...
function useHref(to, { relative } = {}) {
	...
	let { basename, navigator: navigator2 } = import_react.useContext(NavigationContext);
...
function useRoutesImpl(routes, locationArg, dataRouterState, future) {
	...
	let { navigator: navigator2, static: isStatic } = import_react.useContext(NavigationContext);
...
function RouterProvider({ router, flushSync: reactDomFlushSyncImpl }) {
	...
	let navigator2 = import_react.useMemo(() => {
		return {
...
function getSingleFetchDataStrategy(manifest, routeModules, ssr, basename, getRouter) {
	return async (args) => {
		...
		if (request.method !== "GET") return runMiddlewarePipeline(args, false, () => singleFetchActionStrategy(request, matches, basename), (e) => ({ [e.routeId]: {
			type: "error",
			result: e.error
		} }));
		if (!ssr) {
			if (!matches.some((m) => m.shouldLoad && manifest.routes[m.route.id]?.hasLoader && !manifest.routes[m.route.id]?.hasClientLoader)) return runMiddlewarePipeline(args, false, () => nonSsrStrategy(manifest, request, matches, basename), (e) => ({ [e.routeId]: {
				type: "error",
				result: e.error
			} }));
		}
		if (fetcherKey) return runMiddlewarePipeline(args, false, () => singleFetchLoaderFetcherStrategy(request, matches, basename), (e) => ({ [e.routeId]: {
			type: "error",
			result: e.error
		} }));
		return runMiddlewarePipeline(args, false, () => singleFetchLoaderNavigationStrategy(manifest, routeModules, ssr, getRouter(), request, matches, basename), (e) => ({ [e.routeId]: {
			type: "error",
			result: e.error
		} }));
	};
...
async function singleFetchActionStrategy(request, matches, basename) {
	...
	let result = await actionMatch.resolve(async (handler) => {
		return await handler(async () => {
			let { data: data2, status } = await fetchAndDecode(singleFetchUrl(request.url, basename), await createRequestInit(request));
			actionStatus = status;
			return unwrapSingleFetchResult(data2, actionMatch.route.id);
		});
	});
	if (isResponse(result.result) || isRouteErrorResponse(result.result)) return { [actionMatch.route.id]: result };
	...
	let resolvePromise = Promise.all(matches.map(async (m, i) => m.resolve(async (handler) => {
		...
		try {
			let result = await handler(async () => {
				return unwrapSingleFetchResults(await singleFetchDfd.promise, m.route.id);
			});
			results[m.route.id] = {
				type: "data",
				result
...
async function singleFetchLoaderFetcherStrategy(request, matches, basename) {
	...
	let result = await fetcherMatch.resolve(async (handler) => {
		return fetchSingleLoader(handler, stripIndexParam(singleFetchUrl(request.url, basename)), await createRequestInit(request), fetcherMatch.route.id);
	});
...
async function fetchAndDecode(url, init) {
	...
	if ((/* @__PURE__ */ new Set([
		...
		204,
		205
	])).has(res.status)) if (!init.method || init.method === "GET") return {
		status: res.status,
		data: {}
	};
	else return {
...
function getShouldRevalidateFunction(route, manifestRoute, ssr, needsRevalidation) {
	...
	if (!ssr && manifestRoute.hasLoader && !manifestRoute.hasClientLoader) if (route.shouldRevalidate) {
...
function getPatchRoutesOnNavigationFunction(manifest, routeModules, ssr, isSpaMode, basename) {
	if (!isFogOfWarEnabled(ssr)) return;
...
function Links() {
	...
	return /* @__PURE__ */ import_react.createElement(import_react.Fragment, null, typeof criticalCss === "string" ? /* @__PURE__ */ import_react.createElement("style", { dangerouslySetInnerHTML: { __html: criticalCss } }) : null, typeof criticalCss === "object" ? /* @__PURE__ */ import_react.createElement("link", {
		rel: "stylesheet",
...
function PrefetchPageLinksImpl({ page, matches: nextMatches, ...linkProps }) {
	...
	return /* @__PURE__ */ import_react.createElement(import_react.Fragment, null, dataHrefs.map((href2) => /* @__PURE__ */ import_react.createElement("link", {
		key: href2,
		rel: "prefetch",
		as: "fetch",
		href: href2,
		...linkProps
	})), moduleHrefs.map((href2) => /* @__PURE__ */ import_react.createElement("link", {
		key: href2,
		rel: "modulepreload",
		href: href2,
		...linkProps
...
import(${JSON.stringify(manifest.entry.module)});`;
	...
	let preloads = isHydrated ? [] : manifest.entry.imports.concat(getModuleLinkHrefs(matches, manifest, { includeHydrateFallback: true }));
	return isHydrated ? null : /* @__PURE__ */ import_react.createElement(import_react.Fragment, null, !enableFogOfWar ? /* @__PURE__ */ import_react.createElement("link", {
		rel: "modulepreload",
		href: manifest.url,
		...
		crossOrigin: props.crossOrigin
	}), dedupe(preloads).map((path) => /* @__PURE__ */ import_react.createElement("link", {
		key: path,
		rel: "modulepreload",
		href: path,
		crossOrigin: props.crossOrigin
...
try {
	if (isBrowser) window.__reactRouterVersion = "7.3.0";
} catch (e) {}
...
var Link = import_react.forwardRef(function LinkWithRef({ onClick, discover = "render", prefetch = "none", relative, reloadDocument, replace: replace2, state, target, to, preventScrollReset, viewTransition, ...rest }, forwardedRef) {
	...
	return shouldPrefetch && !isAbsolute ? /* @__PURE__ */ import_react.createElement(import_react.Fragment, null, link, /* @__PURE__ */ import_react.createElement(PrefetchPageLinks, { page: href2 })) : link;
});
...
var NavLink = import_react.forwardRef(function NavLinkWithRef({ "aria-current": ariaCurrentProp = "page", caseSensitive = false, className: classNameProp = "", end = false, style: styleProp, to, viewTransition, children, ...rest }, ref) {
	...
	let { navigator: navigator2, basename } = import_react.useContext(NavigationContext);
...
function useFetcher({ key } = {}) {
	...
	let load = import_react.useCallback(async (href2, opts) => {
...
function createRoutesStub(routes, unstable_getContext) {
	return function RoutesTestStub({ initialEntries, initialIndex, hydrationData, future }) {
		...
		let remixContextRef = import_react.useRef();
		if (routerRef.current == null) {
			remixContextRef.current = {
				future: { unstable_middleware: future?.unstable_middleware === true },
				manifest: {
					routes: {},
					entry: {
						imports: [],
			...
			routerRef.current = createMemoryRouter(processRoutes(convertRoutesToDataRoutes(routes, (r) => r), remixContextRef.current.manifest, remixContextRef.current.routeModules), {
				unstable_getContext,
				initialEntries,
				initialIndex,
				hydrationData
		...
		return /* @__PURE__ */ import_react.createElement(FrameworkContext.Provider, { value: remixContextRef.current }, /* @__PURE__ */ import_react.createElement(RouterProvider, { router: routerRef.current }));
	};
}
...
var createRequestHandler = (build, mode) => {
	...
	return async function requestHandler(request, initialContext) {
		...
		if (!_build.ssr) {
			...
			else if (!_build.prerender.includes(normalizedPath) && !_build.prerender.includes(normalizedPath + "/")) if (url.pathname.endsWith(".data")) {
		...
		if (url.pathname.endsWith(".data")) {
			...
			if (_build.getCriticalCss) criticalCss = await _build.getCriticalCss({ pathname });
...
function initSsrInfo() {
	if (!ssrInfo && window.__reactRouterContext && window.__reactRouterManifest && window.__reactRouterRouteModules) ssrInfo = {
...
function createHydratedRouter({ unstable_getContext }) {
	...
	if (ssrInfo.context.isSpaMode) hydrationData = { loaderData };
	else {
		hydrationData = {
		...
		if (initialMatches) for (let match of initialMatches) {
			...
			if (route && manifestRoute && shouldHydrateRouteLoader(manifestRoute, route, ssrInfo.context.isSpaMode) && (route.HydrateFallback || !manifestRoute.hasLoader)) delete hydrationData.loaderData[routeId];
			else if (manifestRoute && !manifestRoute.hasLoader) hydrationData.loaderData[routeId] = null;
		}
		if (hydrationData && hydrationData.errors) hydrationData.errors = deserializeErrors2(hydrationData.errors);
	}
...
function HydratedRouter(props) {
	...
	if (ssrInfo) window.__reactRouterClearCriticalCss = () => setCriticalCss(void 0);
